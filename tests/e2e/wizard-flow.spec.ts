...
test.describe('株式評価ウィザード - E2Eテスト', () => {
  ...
  test.describe('特定会社等フロー', () => {
    test('土地保有比率70%以上の場合、特定会社等に該当する', async ({ page }) => {
      // Step 1: 通常の株主構成
      await page.getByLabel('総議決権数').fill('10000')
      await page.getByRole('button', { name: '株主を追加' }).click()
      await page.getByLabel('氏名').first().fill('田中太郎')
  ...
  test.describe('バリデーションエラー', () => {
    test('Step 1で必須項目が未入力の場合、エラーが表示される', async ({ page }) => {
      await page.getByRole('button', { name: '次へ' }).click()
    ...
    test('Step 2で比率が100%を超える場合、エラーが表示される', async ({ page }) => {
      // Step 1を完了
      await page.getByLabel('総議決権数').fill('10000')
      await page.getByRole('button', { name: '株主を追加' }).click()
      await page.getByLabel('氏名').first().fill('田中太郎')
      ...
      await expect(page.getByText('土地保有比率は0%から100%の範囲で入力してください')).toBeVisible()
    })

    test('Step 4で少数株主で配当利回りが未入力の場合、エラーが表示される', async ({ page }) => {
      // Step 1-3を完了（少数株主を含む）
      await page.getByLabel('総議決権数').fill('10000')
      await page.getByRole('button', { name: '株主を追加' }).click()
      await page.getByLabel('氏名').first().fill('田中太郎')
      ...
      await expect(page.getByText('少数株主の場合は配当利回りを入力してください')).toBeVisible()
    })
  })

  test.describe('状態の永続化', () => {
    test('Step 3でページをリフレッシュしても入力値が保持される', async ({ page }) => {
      // Step 1-2を完了
      await page.getByLabel('総議決権数').fill('10000')
      await page.getByRole('button', { name: '株主を追加' }).click()
      await page.getByLabel('氏名').first().fill('田中太郎')
      ...
      // 入力値が保持されていることを確認
      await expect(page.getByDisplayValue('50')).toBeVisible()
      await expect(page.getByDisplayValue('9000')).toBeVisible()
      await expect(page.getByDisplayValue('100000000')).toBeVisible()
      await expect(page.getByDisplayValue('500000000')).toBeVisible()
      await expect(page.getByDisplayValue('製造業')).toBeVisible()
    })
  })
