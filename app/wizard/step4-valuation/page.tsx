...
export default function Step4ValuationPage() {
  ...
  const [stockPriceSelectionRule, setStockPriceSelectionRule] = useState<
    'lowest3Months' | 'previousYearAverage' | 'last2YearsAverage'
  >('lowest3Months')
  ...
  /**
   * 類似業種比準価額のA（株価）を取得する
   * 通達182条: 課税時期の月・前月・前々月・前年平均・直前2年平均のうち最も低い値を使用
   *
   * 株価選択ルール:
   * - lowest3Months: 課税時期の月・前月・前々月の中から最低値（月次データがある場合）
   * - previousYearAverage: 前年の年平均株価（A フィールド）
   * - last2YearsAverage: 直前2年間の平均株価
   *
   * 実務上は上記すべてのうち最低値を採用するが、ここではUIで選択した方式を返す。
   * 月次データが存在しない月は前年平均（A）にフォールバックする。
   */
  const getAValue = (
    year: string,
    month: string,
    ...
    // 前月・前々月の月次株価を取得（月をまたぐ場合は前年データを参照）
    const getMonthlyValue = (
      ...
      const data = findData(y)
      return data?.monthlyA?.[`${m}月`] ?? null
    }

    if (rule === 'lowest3Months') {
      const current = getMonthlyValue(year, monthNum)
      const prev1 = getMonthlyValue(year, monthNum - 1)
      const prev2 = getMonthlyValue(year, monthNum - 2)
      const values = [current, prev1, prev2].filter(
        (v): v is number => v !== null
      )
      // 月次データがあればその最低値、なければ前年平均にフォールバック
      return values.length > 0 ? Math.min(...values) : industryData.A || 0
    } else if (rule === 'previousYearAverage') {
      // 前年の年平均 = 当年データの A フィールド
      return industryData.A || 0
    } else if (rule === 'last2YearsAverage') {
      // 当年の前年平均 + 前々年の前年平均 → 2年分の平均
      const currentA = industryData.A || 0
      const prevData = findData(`令和${currentYearNumber - 1}年`)
      const prevA = prevData?.A || 0
      if (currentA > 0 && prevA > 0) return (currentA + prevA) / 2
   ...
   *
   * 注: b（配当）= 0 でも計算可能。無配当会社はその比準要素が 0 になるだけ。
   *     c（利益）= 0 でも計算可能（赤字会社等）。
   *     d（純資産）は正の値が必要。
   */
  const calcSimilarIndustryValue = (
    ...
    if (!selectedIndustryData || data.netAssetPerShare <= 0) {
      return 0
    }
    const A = getAValue(
      selectedYear,
      selectedMonth,
      stockPriceSelectionRule,
  ...
  const performRealTimeCalculation = () => {
    ...
    // 純資産価額と業種データが揃えば計算可能（配当・利益は0でも可）
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            <CurrencyInput
              label={
                <div className="flex items-center">
                  1株当たり年配当
                  <Tooltip text="会社の年間の配当金を株式数で割った値です。第4表で計算します。無配当の場合は0を入力してください。" />
                </div>
              }
            ...
            <CurrencyInput
              label={
                <div className="flex items-center">
                  1株当たり年利益
                  <Tooltip text="会社の年間の利益を株式数で割った値です。第4表で計算します。赤字・無利益の場合は0を入力してください。" />
                </div>
              }
              ...
              <div className="flex flex-col space-y-2">
                <label className="inline-flex items-center">
                  <input
                    ...
                    onChange={(e) =>
                      setStockPriceSelectionRule(e.target.value as any)
                    }
                  />
            ...
            {selectedIndustryData && (
              <div className="rounded-lg bg-gray-100 p-4 md:col-span-2">
                ...
                <p className="text-sm text-gray-700">
                  A (年平均株価): {selectedIndustryData.A}
                </p>
                <p className="text-sm text-gray-700">
                  B&apos; (配当): {selectedIndustryData.B_prime}
                </p>
                <p className="text-sm text-gray-700">
                  C&apos; (利益): {selectedIndustryData.C_prime}
                </p>
                <p className="text-sm text-gray-700">
                  D&apos; (純資産): {selectedIndustryData.D_prime}
                </p>
                {selectedIndustryData.monthlyA && (
                  <div className="mt-2">
                    ...
                    <div className="grid grid-cols-4 gap-1 text-xs text-gray-600">
                      {Object.entries(selectedIndustryData.monthlyA).map(
                        ([month, value]) => (
                          <span key={month}>
                            {month}: {value}
                          </span>
            ...
            <div className="space-y-2 text-sm">
              ...
              {companySize === 'medium' && (
                <div>
                  ...
                  <p className="text-xs text-gray-600">
                    評価式: min(類似業種比準価額 × {lRatio} +
                    純資産価額(80%相当額) × (1 - {lRatio}), 純資産価額)
                  </p>
                </div>
              )}
              {companySize === 'small' && (
                <div>
                  ...
                  <p className="text-xs text-gray-600">
                    原則: 純資産価額 / 併用: min(類似業種比準 × 0.5 + 純資産 ×
                    0.5, 純資産価額)
                  </p>
                </div>
              )}
